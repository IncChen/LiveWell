...
const CGFloat   LPPageVCSegmentIndicatorHeightLine  = 2.0f;
...
#pragma mark - 创建editButton
    // edit按钮的背景视图
    _editBgView = [[UIControl alloc] init];
    [_editBgView addTarget:self action:@selector(editButtonAction) forControlEvents:UIControlEventTouchUpInside];
    _editBgView.backgroundColor = _segmentScrollView.backgroundColor;
    [self.view addSubview:_editBgView];
    
    [_editBgView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [editButton setBackgroundImage:[UIImage imageNamed:@"ic_thumbnail"] forState:UIControlStateNormal];
    [editButton addTarget:self action:@selector(editButtonAction) forControlEvents:UIControlEventTouchUpInside];
    editButton.backgroundColor = [UIColor clearColor];
    [_editBgView addSubview:editButton];
    
    [editButton mas_makeConstraints:^(MASConstraintMaker *make) {
        
        // 这就简单了 == editBgView
        make.center.mas_equalTo(_editBgView);
    }];
...
- (void)reloadData {
    ...
    if (self.contentStyle == LPPageVCContentStyleDefault) {
//        [_editBgView removeFromSuperview];
    ...
    for (NSInteger index = 0; index < _numberOfContent; ++index) {
        ...
        // 改进适配字数
        CGSize size = [label.text sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13.0f]}];
        self.size = size;
        ...
        [label mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            sizeTest.width = sizeTest.width + 22;
